...
export default function ProjectsIdPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const url = "/api/projects/{id}".replace('{id}', encodeURIComponent(id || ''));
  ...
  return (
    <div>
      <PageHeader
        title="مشروع"
        description="بيانات المشروع + المهام + الميزانية"
        actions={
          <Link href="/projects">
          ...
          <Card className="p-6">
            ...
            <dl className="space-y-3">
              <div className="flex justify-between text-sm gap-2">
                <dt className="text-gray-500 flex-shrink-0">code</dt>
                <dd className="font-medium text-gray-800 font-mono text-xs text-end break-all">
                  {String(item.code ?? item['codeName'] ?? '—')}
                </dd>
              </div>
              <div className="flex justify-between text-sm gap-2">
              ...
              <div className="flex justify-between text-sm gap-2">
                <dt className="text-gray-500 flex-shrink-0">customerName</dt>
                <dd className="font-medium text-gray-800 font-mono text-xs text-end break-all">
                  {String(item.customerName ?? item['customerNameName'] ?? '—')}
                </dd>
              </div>
              <div className="flex justify-between text-sm gap-2">
                <dt className="text-gray-500 flex-shrink-0">startDate</dt>
                <dd className="font-medium text-gray-800 font-mono text-xs text-end break-all">
                  {String(item.startDate ?? item['startDateName'] ?? '—')}
                </dd>
              </div>
              <div className="flex justify-between text-sm gap-2">
                <dt className="text-gray-500 flex-shrink-0">endDate</dt>
                <dd className="font-medium text-gray-800 font-mono text-xs text-end break-all">
                  {String(item.endDate ?? item['endDateName'] ?? '—')}
                </dd>
              </div>
              <div className="flex justify-between text-sm gap-2">
              ...
              <div className="flex justify-between text-sm gap-2">
                <dt className="text-gray-500 flex-shrink-0">budget</dt>
                <dd className="font-medium text-gray-800 font-mono text-xs text-end break-all">
                  {String(item.budget ?? item['budgetName'] ?? '—')}
                </dd>
              </div>
              <div className="flex justify-between text-sm gap-2">
                <dt className="text-gray-500 flex-shrink-0">actualCost</dt>
                <dd className="font-medium text-gray-800 font-mono text-xs text-end break-all">
                  {String(item.actualCost ?? item['actualCostName'] ?? '—')}
                </dd>
              </div>
              <div className="flex justify-between text-sm gap-2">
                <dt className="text-gray-500 flex-shrink-0">progress</dt>
                <dd className="font-medium text-gray-800 font-mono text-xs text-end break-all">
                  {String(item.progress ?? item['progressName'] ?? '—')}
                </dd>
              </div>
              <div className="flex justify-between text-sm gap-2">
