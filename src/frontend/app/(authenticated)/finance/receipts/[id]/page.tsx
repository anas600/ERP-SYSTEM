...
export default function FinanceReceiptsIdPage() {
  ...
  const load = async () => {
    ...
    try {
      const url = "/api/ar/receipts/{id}".replace('{id}', encodeURIComponent(id || ''));
  ...
  return (
    <div>
      <PageHeader
        title="سند قبض"
        description="بيانات سند القبض + التوزيعات"
        actions={
          <Link href="/finance/receipts">
            <Button variant="secondary" iconLeft={<ArrowRight className="h-4 w-4" />}>العودة إلى سندات القبض</Button>
          </Link>
        }
      />
          ...
          <Card className="p-6">
            ...
            <dl className="space-y-3">
              <div className="flex justify-between text-sm gap-2">
                <dt className="text-gray-500 flex-shrink-0">receiptNumber</dt>
                <dd className="font-medium text-gray-800 font-mono text-xs text-end break-all">
                  {String(item.receiptNumber ?? item['receiptNumberName'] ?? '—')}
                </dd>
              </div>
              <div className="flex justify-between text-sm gap-2">
                <dt className="text-gray-500 flex-shrink-0">receiptDate</dt>
                <dd className="font-medium text-gray-800 font-mono text-xs text-end break-all">
                  {String(item.receiptDate ?? item['receiptDateName'] ?? '—')}
                </dd>
              </div>
              <div className="flex justify-between text-sm gap-2">
              ...
              <div className="flex justify-between text-sm gap-2">
                ...
                <dd className="font-medium text-gray-800 font-mono text-xs text-end break-all">
                  {String(item.amount ?? item['amountName'] ?? '—')}
                </dd>
              </div>
              <div className="flex justify-between text-sm gap-2">
              ...
              <div className="flex justify-between text-sm gap-2">
                ...
                <dd className="font-medium text-gray-800 font-mono text-xs text-end break-all">
                  {String(item.method ?? item['methodName'] ?? '—')}
                </dd>
              </div>
              <div className="flex justify-between text-sm gap-2">
