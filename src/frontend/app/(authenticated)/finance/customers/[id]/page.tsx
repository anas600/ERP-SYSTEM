...
export default function FinanceCustomersIdPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const url = "/api/ar/customers/{id}".replace('{id}', encodeURIComponent(id || ''));
      ...
      <PageHeader
        ...
        actions={
          <Link href="/finance/customers">
            <Button variant="secondary" iconLeft={<ArrowRight className="h-4 w-4" />}>العودة إلى العملاء</Button>
          </Link>
        }
      />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <Card className="p-6">
            ...
            <dl className="space-y-3">
              ...
              <div className="flex justify-between text-sm gap-2">
                <dt className="text-gray-500 flex-shrink-0">address</dt>
                <dd className="font-medium text-gray-800 font-mono text-xs text-end break-all">
                  {String(item.address ?? item['addressName'] ?? '—')}
                </dd>
              </div>
              <div className="flex justify-between text-sm gap-2">
                <dt className="text-gray-500 flex-shrink-0">creditLimit</dt>
                <dd className="font-medium text-gray-800 font-mono text-xs text-end break-all">
                  {String(item.creditLimit ?? item['creditLimitName'] ?? '—')}
                </dd>
              </div>
              <div className="flex justify-between text-sm gap-2">
                <dt className="text-gray-500 flex-shrink-0">paymentTermsDays</dt>
                <dd className="font-medium text-gray-800 font-mono text-xs text-end break-all">
                  {String(item.paymentTermsDays ?? item['paymentTermsDaysName'] ?? '—')}
                </dd>
              </div>
              <div className="flex justify-between text-sm gap-2">
