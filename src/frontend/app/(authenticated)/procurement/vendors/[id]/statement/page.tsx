...
// Sprint 36 (DEC-122): Vendor Statement (كشف حساب مورّد)
// Opening balance + chronological bills/payments + closing balance
// AP convention: positive balance = we owe vendor
...
export default function VendorStatementPage() {
  ...
  const load = async () => {
    ...
    setError(null);
    try {
      const r = await procurementApi.getVendorStatement(id, from, to);
      setData(r);
    } catch (e: unknown) {
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
            <Card className="p-4">
              ...
              <p className="text-[10px] text-gray-400 mt-1">مستحق قبل الفترة</p>
            </Card>
            <Card className="p-4">
              <p className="text-xs text-gray-500 mb-1">إجمالي الفواتير</p>
              <p className="text-xl font-bold font-mono text-orange-700">
                {formatNumber(totals?.billed ?? 0)} <span className="text-sm text-gray-500">LYD</span>
              </p>
            </Card>
            <Card className="p-4">
              <p className="text-xs text-gray-500 mb-1">إجمالي المدفوعات</p>
              <p className="text-xl font-bold font-mono text-green-700">
                {formatNumber(totals?.paid ?? 0)} <span className="text-sm text-gray-500">LYD</span>
              </p>
            </Card>
            <Card
              className={`p-4 ${
                (totals?.closing ?? 0) > 0
                  ? 'border-r-4 border-orange-400 bg-orange-50/30'
                  : (totals?.closing ?? 0) < 0
          ...
          <p className="mt-3 text-xs text-gray-500">
            إجمالي الحركات: <span className="font-semibold">{data.lines.length}</span> • موجب = مستحق للمورّد
            (علينا دين) • سالب = لنا رصيد دائن
          </p>
        </>
      )}
    </div>
