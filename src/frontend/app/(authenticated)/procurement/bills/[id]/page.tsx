...
export default function ProcurementBillsIdPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const url = "/api/procurement/bills/{id}".replace('{id}', encodeURIComponent(id || ''));
  ...
  return (
    <div>
      <PageHeader
        title="فاتورة مورد"
        description="بيانات فاتورة المورد + البنود + الدفعات"
        actions={
          <Link href="/procurement/bills">
            <Button variant="secondary" iconLeft={<ArrowRight className="h-4 w-4" />}>العودة إلى فواتير الموردين</Button>
          </Link>
        }
      />
          ...
          <Card className="p-6">
            ...
            <dl className="space-y-3">
              <div className="flex justify-between text-sm gap-2">
                <dt className="text-gray-500 flex-shrink-0">billNumber</dt>
                <dd className="font-medium text-gray-800 font-mono text-xs text-end break-all">
                  {String(item.billNumber ?? item['billNumberName'] ?? '—')}
                </dd>
              </div>
              <div className="flex justify-between text-sm gap-2">
                <dt className="text-gray-500 flex-shrink-0">billDate</dt>
                <dd className="font-medium text-gray-800 font-mono text-xs text-end break-all">
                  {String(item.billDate ?? item['billDateName'] ?? '—')}
                </dd>
              </div>
              <div className="flex justify-between text-sm gap-2">
                <dt className="text-gray-500 flex-shrink-0">dueDate</dt>
                <dd className="font-medium text-gray-800 font-mono text-xs text-end break-all">
                  {String(item.dueDate ?? item['dueDateName'] ?? '—')}
                </dd>
              </div>
              <div className="flex justify-between text-sm gap-2">
              ...
              <div className="flex justify-between text-sm gap-2">
                <dt className="text-gray-500 flex-shrink-0">subtotal</dt>
                <dd className="font-medium text-gray-800 font-mono text-xs text-end break-all">
                  {String(item.subtotal ?? item['subtotalName'] ?? '—')}
                </dd>
              </div>
              <div className="flex justify-between text-sm gap-2">
                <dt className="text-gray-500 flex-shrink-0">vatAmount</dt>
                <dd className="font-medium text-gray-800 font-mono text-xs text-end break-all">
                  {String(item.vatAmount ?? item['vatAmountName'] ?? '—')}
                </dd>
              </div>
              <div className="flex justify-between text-sm gap-2">
                <dt className="text-gray-500 flex-shrink-0">total</dt>
                <dd className="font-medium text-gray-800 font-mono text-xs text-end break-all">
                  {String(item.total ?? item['totalName'] ?? '—')}
                </dd>
              </div>
              <div className="flex justify-between text-sm gap-2">
