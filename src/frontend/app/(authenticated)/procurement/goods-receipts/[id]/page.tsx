...
export default function ProcurementGoodsReceiptsIdPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const url = "/api/procurement/grs/{id}".replace('{id}', encodeURIComponent(id || ''));
  ...
  return (
    <div>
      <PageHeader
        title="استلام بضاعة"
        description="بيانات استلام البضاعة + البنود"
        actions={
          <Link href="/procurement/goods-receipts">
            <Button variant="secondary" iconLeft={<ArrowRight className="h-4 w-4" />}>العودة إلى استلامات البضاعة</Button>
          </Link>
        }
      />
          ...
          <Card className="p-6">
            ...
            <dl className="space-y-3">
              <div className="flex justify-between text-sm gap-2">
                <dt className="text-gray-500 flex-shrink-0">grNumber</dt>
                <dd className="font-medium text-gray-800 font-mono text-xs text-end break-all">
                  {String(item.grNumber ?? item['grNumberName'] ?? '—')}
                </dd>
              </div>
              <div className="flex justify-between text-sm gap-2">
                <dt className="text-gray-500 flex-shrink-0">receivedDate</dt>
                <dd className="font-medium text-gray-800 font-mono text-xs text-end break-all">
                  {String(item.receivedDate ?? item['receivedDateName'] ?? '—')}
                </dd>
              </div>
              <div className="flex justify-between text-sm gap-2">
