...
public class DefaultCoASeedTests
{
    ...
    public async Task EnsureDefaultCoAAsync_SeedsAllAccounts()
    {
        ...
        await accountRepo.EnsureDefaultCoAAsync(companyId, CancellationToken.None);
        var accounts = await accountRepo.ListByCompanyAsync(companyId, CancellationToken.None);
    ...
    public async Task EnsureDefaultCoAAsync_IsIdempotent()
    {
        ...
        await accountRepo.EnsureDefaultCoAAsync(companyId, CancellationToken.None);
        var first = (await accountRepo.ListByCompanyAsync(companyId, CancellationToken.None)).Count;
        await accountRepo.EnsureDefaultCoAAsync(companyId, CancellationToken.None);
        var second = (await accountRepo.ListByCompanyAsync(companyId, CancellationToken.None)).Count;
    ...
    public async Task CloneCoAFromCompany_PreservesStructureAndFlags()
    {
        ...
        foreach (var (code, name, type, parentCode, postable, intercompany) in DefaultCoASeed.HoldingAccounts)
        {
            ...
            {
                Id = id, CompanyId = sourceId, Code = code, Name = name,
        ...
        var cloned = await accountRepo.ListByCompanyAsync(targetId, CancellationToken.None);
...
internal class FakeAccountRepository : IAccountRepository
{
    ...
    public Task<IReadOnlyList<Account>> ListByCompanyAsync(Guid? companyId, CancellationToken ct) =>
        Task.FromResult<IReadOnlyList<Account>>(_accounts.Where(a => a.CompanyId == companyId).ToList());
    ...
    public async Task CloneCoAFromCompanyAsync(Guid targetCompanyId, Guid sourceCompanyId, CancellationToken ct)
    {
        ...
        foreach (var a in src.Where(a => a.ParentAccountId == null))
            _accounts.Add(new Account { Id = idMap[a.Id], CompanyId = targetCompanyId, Code = a.Code, Name = a.Name, Type = a.Type, NormalBalance = a.NormalBalance, IsPostable = a.IsPostable, IsActive = a.IsActive, IsIntercompany = a.IsIntercompany, CreatedAt = DateTime.UtcNow, UpdatedAt = DateTime.UtcNow });
        foreach (var a in src.Where(a => a.ParentAccountId != null))
        {
            Guid? newParent = a.ParentAccountId.HasValue && idMap.TryGetValue(a.ParentAccountId.Value, out var p) ? p : null;
            _accounts.Add(new Account { Id = idMap[a.Id], CompanyId = targetCompanyId, Code = a.Code, Name = a.Name, Type = a.Type, NormalBalance = a.NormalBalance, ParentAccountId = newParent, IsPostable = a.IsPostable, IsActive = a.IsActive, IsIntercompany = a.IsIntercompany, CreatedAt = DateTime.UtcNow, UpdatedAt = DateTime.UtcNow });
        }
