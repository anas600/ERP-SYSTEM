...
public class StockMovementServiceTests
{
    ...
    private sealed class Fixture
    {
        ...
        public Item AddItem(string sku = "TEST", decimal reorder = 0, decimal reorderQty = 0)
        {
            var i = new Item
            {
                Id = Guid.NewGuid(), CompanyId = Guid.NewGuid(), Sku = sku, Name = "صنف اختبار",
    ...
    public async Task CreateReceive_StoresAsDraft_WithPositiveQuantity()
    {
        var f = new Fixture();
        var item = f.AddItem();
        var r = await f.Svc.CreateReceiveAsync(Guid.NewGuid(), new ReceiveStockRequest
        {
    ...
    public async Task Post_Receive_UpdatesStockLevel_WithMovingAverage()
    {
        ...
        var post = await f.Svc.PostAsync(Guid.NewGuid(), r.Value!.Id, CancellationToken.None);
    ...
    public async Task Post_SecondReceive_RecomputesMovingAverage()
    {
        var f = new Fixture();
        var item = f.AddItem();
        var r1 = await f.Svc.CreateReceiveAsync(Guid.NewGuid(), new ReceiveStockRequest
        {
            CompanyId = Guid.NewGuid(), Reference = "RCV-1", MovementDate = DateTime.UtcNow,
            ItemId = item.Id, WarehouseId = Guid.NewGuid(), Quantity = 100, UnitCost = 10
        }, CancellationToken.None);
        await f.Svc.PostAsync(Guid.NewGuid(), r1.Value!.Id, CancellationToken.None);
        var r2 = await f.Svc.CreateReceiveAsync(Guid.NewGuid(), new ReceiveStockRequest
        {
        ...
        await f.Svc.PostAsync(Guid.NewGuid(), r2.Value!.Id, CancellationToken.None);
        var level = f.Levels.Levels.Values.First();
    ...
    public async Task Post_Issue_DecreasesStockLevel()
    {
        ...
        await f.Svc.PostAsync(Guid.NewGuid(), r1.Value!.Id, CancellationToken.None);
        var r2 = await f.Svc.CreateIssueAsync(Guid.NewGuid(), new IssueStockRequest
        {
        ...
        var post = await f.Svc.PostAsync(Guid.NewGuid(), r2.Value!.Id, CancellationToken.None);
        post.Succeeded.Should().BeTrue();
        var level = f.Levels.Levels.Values.First();
    ...
    public async Task PostIssue_InsufficientStock_FailsAtPost()
    {
        ...
        var draft = await f.Svc.CreateIssueAsync(Guid.NewGuid(), new IssueStockRequest
        {
            CompanyId = Guid.NewGuid(), Reference = "ISS", MovementDate = DateTime.UtcNow,
        ...
        var post = await f.Svc.PostAsync(Guid.NewGuid(), draft.Value!.Id, CancellationToken.None);
    ...
    public async Task Post_Transfer_UpdatesBothWarehousesAtomically()
    {
        ...
        await f.Svc.PostAsync(Guid.NewGuid(), r1.Value!.Id, CancellationToken.None);
        var r2 = await f.Svc.CreateTransferAsync(Guid.NewGuid(), new TransferStockRequest
        {
    ...
    public async Task Post_DuplicateReference_Fails()
    {
        var f = new Fixture();
        var item = f.AddItem();
        await f.Svc.CreateReceiveAsync(Guid.NewGuid(), new ReceiveStockRequest
        {
    ...
    public async Task Post_AlreadyPosted_Fails()
    {
        ...
        await f.Svc.PostAsync(Guid.NewGuid(), r.Value!.Id, CancellationToken.None);
        var second = await f.Svc.PostAsync(Guid.NewGuid(), r.Value!.Id, CancellationToken.None);
    ...
    public async Task Reverse_CreatesOpposite_MarksOriginalAsReversed()
    {
        ...
        await f.Svc.PostAsync(Guid.NewGuid(), r1.Value!.Id, CancellationToken.None);
        var reverse = await f.Svc.ReverseAsync(Guid.NewGuid(), r1.Value!.Id, "تصحيح", CancellationToken.None);
        ...
        var orig = await f.Svc.GetByIdAsync(r1.Value!.Id, CancellationToken.None);
    ...
    public async Task Post_Adjust_Negative_AllowedWithinStock()
    {
        ...
        await f.Svc.PostAsync(Guid.NewGuid(), r1.Value!.Id, CancellationToken.None);
        var adj = await f.Svc.CreateAdjustAsync(Guid.NewGuid(), new AdjustStockRequest
        {
        ...
        var post = await f.Svc.PostAsync(Guid.NewGuid(), adj.Value!.Id, CancellationToken.None);
        post.Succeeded.Should().BeTrue();
...
internal class FakeStockMovementRepo : IStockMovementRepository
{
    ...
    public Task<StockMovement?> GetByReferenceAsync(string reference, CancellationToken ct) =>
        Task.FromResult(_items.Values.FirstOrDefault(m => m.Reference == reference));
    public Task<IReadOnlyList<StockMovement>> ListAsync(Guid? companyId, StockMovementType? type, StockMovementStatus? status, int skip, int take, CancellationToken ct) =>
        Task.FromResult<IReadOnlyList<StockMovement>>(_items.Values
            .Where(m => (companyId == null || m.CompanyId == companyId)
...
internal class FakeStockLevelRepo : IStockLevelRepository
{
    ...
    public Task<IReadOnlyList<StockLevel>> GetLowStockAsync(Guid companyId, CancellationToken ct) =>
        Task.FromResult<IReadOnlyList<StockLevel>>(Levels.Values.Where(l => l.CompanyId == companyId).ToList());
...
internal class FakeItemRepo : IItemRepository
{
    ...
    public Task<Item?> GetBySkuAsync(string sku, CancellationToken ct) =>
        Task.FromResult(Items.Values.FirstOrDefault(i => i.Sku == sku));
    public Task<Item?> GetByBarcodeAsync(string barcode, CancellationToken ct) =>
        Task.FromResult(barcode == null ? null : Items.Values.FirstOrDefault(i => i.Barcode == barcode));
...
internal class FakeStockReservationRepo : IStockReservationRepository
{
    ...
    public Task<IReadOnlyList<StockReservation>> ListAsync(Guid? itemId, Guid? warehouseId, CancellationToken ct) =>
        Task.FromResult<IReadOnlyList<StockReservation>>(_items.Values
            .Where(r => (itemId == null || r.ItemId == itemId) && (warehouseId == null || r.WarehouseId == warehouseId))
            .ToList());
    public Task<IReadOnlyList<StockReservation>> GetByReferenceAsync(string referenceType, Guid referenceId, CancellationToken ct) =>
        Task.FromResult<IReadOnlyList<StockReservation>>(_items.Values
            .Where(r => r.ReferenceType == referenceType && r.ReferenceId == referenceId).ToList());
...
internal class FakeNotificationService : INotificationService
{
    ...
    public Task CreateAsync(Guid userId, string type, string title, string message, string? referenceType = null, Guid? referenceId = null)
    {
    ...
    public Task<IReadOnlyList<ERPSystem.Modules.Notifications.Entities.Notification>> ListAsync(Guid userId, bool unreadOnly, int skip, int take, CancellationToken ct) =>
        Task.FromResult<IReadOnlyList<ERPSystem.Modules.Notifications.Entities.Notification>>(Created);
    public Task<int> CountUnreadAsync(Guid userId, CancellationToken ct) => Task.FromResult(Created.Count(n => !n.IsRead));
    public Task MarkReadAsync(Guid userId, Guid id, CancellationToken ct) { return Task.CompletedTask; }
}
