...
{
    Task<CashFlowResponse> GetAsync(Guid companyId, DateTime from, DateTime to, CancellationToken ct);
}
...
public sealed class CashFlowService : ICashFlowService
{
    ...
    public async Task<CashFlowResponse> GetAsync(Guid companyId, DateTime from, DateTime to, CancellationToken ct)
    {
        ...
        const string periodSumSql = @"
            ...
            WHERE je.company_id = @CompanyId AND je.status = 2
        ...
        var periodRows = (await conn.QueryAsync<(int AccountType, string AccountCode, decimal Dr, decimal Cr)>(
            new CommandDefinition(periodSumSql, new { CompanyId = companyId, From = from, To = to }, cancellationToken: ct))).ToList();
        ...
        var wcDelta = await GetNetChangeAsync(conn, companyId, from, to, "1230", ct)
                    - await GetNetChangeAsync(conn, companyId, from, to, "2210", ct)
                    - await GetNetChangeAsync(conn, companyId, from, to, "1240", ct);

        var operating = new CashFlowSection();
        operating.Lines.Add(new CashFlowLine { Description = "صافي الربح", Amount = netProfit });
        operating.Lines.Add(new CashFlowLine { Description = "Δ الذمم المدينة (1230)", Amount = -await GetNetChangeAsync(conn, companyId, from, to, "1230", ct) });
        operating.Lines.Add(new CashFlowLine { Description = "Δ الذمم الدائنة (2210)", Amount = await GetNetChangeAsync(conn, companyId, from, to, "2210", ct) });
        operating.Lines.Add(new CashFlowLine { Description = "Δ المخزون (1240)", Amount = -await GetNetChangeAsync(conn, companyId, from, to, "1240", ct) });
        ...
        {
            var change = await GetNetChangeAsync(conn, companyId, from, to, faCode, ct);
            if (change != 0)
        ...
        {
            var change = await GetNetChangeAsync(conn, companyId, from, to, code, ct);
            if (change != 0)
    ...
    private static async Task<decimal> GetNetChangeAsync(
        System.Data.IDbConnection conn, Guid companyId, DateTime from, DateTime to, string accountCode, CancellationToken ct)
    {
        const string sql = @"
        ...
        var delta = await conn.QueryFirstOrDefaultAsync<decimal?>(new CommandDefinition(sql,
            new { CompanyId = companyId, Code = accountCode, From = from, To = to }, cancellationToken: ct));
        return delta ?? 0m;
    }
}
