...
public sealed class PostingRuleRepository : IPostingRuleRepository
{
    ...
    public async Task<IReadOnlyList<PostingRule>> ListActiveByEventAsync(TriggeringEvent eventType, CancellationToken ct)
    {
        using var conn = await _db.CreateOltpConnectionAsync(ct);
        const string sql = @"
            SELECT id, company_id AS CompanyId, name, description, event_type AS EventType,
        ...
        var rows = await conn.QueryAsync<PostingRule>(new CommandDefinition(sql,
            new { EventType = (int)eventType }, cancellationToken: ct));
        return rows.AsList();
    }
    ...
    public async Task<IReadOnlyList<PostingRule>> ListAsync(CancellationToken ct)
    {
        ...
        var rows = await conn.QueryAsync<PostingRule>(new CommandDefinition(sql, cancellationToken: ct));
        return rows.AsList();
    }
