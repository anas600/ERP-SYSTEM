...
public sealed class StockLevelRepository : IStockLevelRepository
{
    ...
    public async Task<StockLevel?> GetAsync(Guid itemId, Guid warehouseId, CancellationToken ct)
    {
        using var conn = await _db.CreateOltpConnectionAsync(ct);
        return await conn.QueryFirstOrDefaultAsync<StockLevel>(new CommandDefinition(
            $"SELECT {Sel} FROM stock_levels WHERE item_id = @ItemId AND warehouse_id = @WarehouseId LIMIT 1",
            new { ItemId = itemId, WarehouseId = warehouseId }, cancellationToken: ct));
    }

    public async Task<IReadOnlyList<StockLevel>> GetByItemAsync(Guid itemId, CancellationToken ct)
    {
        using var conn = await _db.CreateOltpConnectionAsync(ct);
        var rows = await conn.QueryAsync<StockLevel>(new CommandDefinition(
            $"SELECT {Sel} FROM stock_levels WHERE item_id = @ItemId ORDER BY warehouse_id",
            new { ItemId = itemId }, cancellationToken: ct));
        return rows.AsList();
    }

    public async Task<IReadOnlyList<StockLevel>> GetByWarehouseAsync(Guid warehouseId, CancellationToken ct)
    {
        using var conn = await _db.CreateOltpConnectionAsync(ct);
        var rows = await conn.QueryAsync<StockLevel>(new CommandDefinition(
            $"SELECT {Sel} FROM stock_levels WHERE warehouse_id = @WarehouseId ORDER BY item_id",
            new { WarehouseId = warehouseId }, cancellationToken: ct));
        return rows.AsList();
    }
    ...
    /// </summary>
    public async Task<IReadOnlyList<StockLevel>> GetLowStockAsync(Guid companyId, CancellationToken ct)
    {
        using var conn = await _db.CreateOltpConnectionAsync(ct);
        const string sql = @"
            SELECT sl.id, sl.company_id AS CompanyId, sl.item_id AS ItemId, sl.warehouse_id AS WarehouseId,
        ...
        var rows = await conn.QueryAsync<StockLevel>(new CommandDefinition(sql, new { CompanyId = companyId }, cancellationToken: ct));
        return rows.AsList();
    }
    ...
    public async Task UpsertAsync(StockLevel level, int expectedVersion, CancellationToken ct)
    {
        ...
        if (rows == 0)
        {
            ...
            await conn.ExecuteAsync(new CommandDefinition(@"
                INSERT INTO stock_levels (id, company_id, item_id, warehouse_id,
                                          quantity_on_hand, quantity_reserved, average_cost, last_movement_at, version)
                VALUES (@Id, @CompanyId, @ItemId, @WarehouseId,
