...
public sealed class CostCenterRepository : ICostCenterRepository
{
    ...
    private const string SelectColumns = @"id, company_id AS CompanyId, code, name, type, parent_id AS ParentId,
    ...
    public async Task<CostCenter?> GetByCodeAsync(string code, CancellationToken ct)
    {
        using var conn = await _db.CreateOltpConnectionAsync(ct);
        return await conn.QueryFirstOrDefaultAsync<CostCenter>(new CommandDefinition(
            $"SELECT {SelectColumns} FROM cost_centers WHERE LOWER(code) = LOWER(@Code) LIMIT 1",
            new { Code = code }, cancellationToken: ct));
    }

    public async Task<IReadOnlyList<CostCenter>> ListAsync(Guid? companyId, CostCenterType? type, bool includeInactive, CancellationToken ct)
    {
        using var conn = await _db.CreateOltpConnectionAsync(ct);
        var sql = $"SELECT {SelectColumns} FROM cost_centers WHERE 1=1";
    ...
    public async Task InsertAsync(CostCenter cc, CancellationToken ct)
    {
        ...
        await conn.ExecuteAsync(new CommandDefinition(@"
            INSERT INTO cost_centers (id, company_id, code, name, type, parent_id,
                                      budget_amount, start_date, end_date, sku, location, activity_category,
                                      is_active, created_at, updated_at)
            VALUES (@Id, @CompanyId, @Code, @Name, @Type, @ParentId,
