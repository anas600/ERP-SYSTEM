...
public sealed class NotificationRepository : INotificationRepository
{
    ...
    private const string Sel = @"id, user_id AS UserId, type, title, message,
    ...
    public async Task InsertAsync(Notification n, CancellationToken ct)
    {
        ...
        await conn.ExecuteAsync(new CommandDefinition(@"
            INSERT INTO notifications (id, user_id, type, title, message, reference_type, reference_id, is_read, created_at, read_at)
            VALUES (@Id, @UserId, @Type, @Title, @Message, @ReferenceType, @ReferenceId, @IsRead, @CreatedAt, @ReadAt)", n, cancellationToken: ct));
    }
    ...
    {
        using var conn = await _db.CreateOltpConnectionAsync(ct);
        var sql = $"SELECT {Sel} FROM notifications WHERE user_id = @UserId"
