...
{
    Task<ProjectPnL> GetProjectPnLAsync(Guid projectId, DateTime from, DateTime to, CancellationToken ct);
    Task<ProjectBudgetVsActual> GetBudgetVsActualAsync(Guid projectId, CancellationToken ct);
    Task<List<ProjectSummary>> GetProjectsSummaryAsync(Guid? companyId, CancellationToken ct);
}
...
public sealed class ProjectReportService : IProjectReportService
{
    ...
    /// </summary>
    public async Task<ProjectPnL> GetProjectPnLAsync(Guid projectId, DateTime from, DateTime to, CancellationToken ct)
    {
        ...
        // joined to journal_entries (status=Posted, date range)
        ...
        var rows = (await conn.QueryAsync<(string AccountCode, int AccountType, decimal TotalDebit, decimal TotalCredit)>(
            new CommandDefinition(sql, new { CostCenterId = project.CostCenterId, From = from, To = to }, cancellationToken: ct))).ToList();
    ...
    public async Task<ProjectBudgetVsActual> GetBudgetVsActualAsync(Guid projectId, CancellationToken ct)
    {
    ...
    public async Task<List<ProjectSummary>> GetProjectsSummaryAsync(Guid? companyId, CancellationToken ct)
    {
        using var conn = await _db.CreateOltpConnectionAsync(ct);
        var sql = @"
