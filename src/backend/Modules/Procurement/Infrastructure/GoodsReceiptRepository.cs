...
public sealed class GoodsReceiptRepository : IGoodsReceiptRepository
{
    ...
    private const string SelLine = @"id, goods_receipt_id AS GoodsReceiptId,
    ...
    public async Task<GoodsReceipt?> GetByGrNumberAsync(string grNumber, CancellationToken ct)
    {
        using var conn = await _db.CreateOltpConnectionAsync(ct);
        return await conn.QueryFirstOrDefaultAsync<GoodsReceipt>(new CommandDefinition(
            $"SELECT {SelGr} FROM goods_receipts WHERE gr_number = @GrNumber LIMIT 1",
            new { GrNumber = grNumber }, cancellationToken: ct));
    }

    public async Task<IReadOnlyList<GoodsReceipt>> ListAsync(Guid? poId, GoodsReceiptStatus? status, int skip, int take, CancellationToken ct)
    {
        using var conn = await _db.CreateOltpConnectionAsync(ct);
        var sql = $"SELECT {SelGr} FROM goods_receipts WHERE 1=1";
    ...
    public async Task InsertLinesAsync(Guid grId, IEnumerable<GoodsReceiptLine> lines, CancellationToken ct)
    {
        using var conn = await _db.CreateOltpConnectionAsync(ct);
        foreach (var l in lines)
        {
            await conn.ExecuteAsync(new CommandDefinition(@"
                INSERT INTO goods_receipt_lines (id, goods_receipt_id, item_id, quantity, unit_cost, notes, line_order)
                VALUES (@Id, @GoodsReceiptId, @ItemId, @Quantity, @UnitCost, @Notes, @LineOrder)",
                new
                {
                    l.Id, GoodsReceiptId = grId,
